...
class RoutineDeductionAdmin(admin.ModelAdmin):
    list_display = ("team_entry", "deduction_type", "entered_by", "count", "judges_reporting")
    list_filter = ("deduction_type__penalty_type", "entered_by")
    search_fields = ("team_entry__school__name", "deduction_type__code")

    def has_add_permission(self, request):
        return request.user.roles.filter(name="Superior Judge").exists()

    def has_change_permission(self, request, obj=None):
        # Tabulators can view but not edit
